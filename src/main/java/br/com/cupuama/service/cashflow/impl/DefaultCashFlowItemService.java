...
import br.com.cupuama.service.cashflow.CashFlowItemService;
 ...
 */
@Service
public class DefaultCashFlowItemService implements CashFlowItemService {

	public static final SimpleDateFormat PERIOD_DATE_FORMAT = new SimpleDateFormat("yyyyMM");
	private static final Logger LOG = LoggerFactory.getLogger(DefaultCashFlowItemService.class);
	...
	public DefaultCashFlowItemService(final CashFlowItemRepository cashFlowItemRepository) {
		this.cashFlowItemRepository = cashFlowItemRepository;
	}
	
	/**
	 * Locate a CashFlowItem by its ID
	 * 
	 ...
	 */
	@Override
	public CashFlowItem find(Long cashFlowItemId) throws EntityNotFoundException {
		return findCashFlowItemChecked(cashFlowItemId);
	}
	...
	public CashFlowItem addCashFlowItem(CashFlowItem cashFlowItemDO) throws ConstraintsViolationException, EntityNotFoundException {
		...
		try {
			cashFlowItem = cashFlowItemRepository.save(cashFlowItemDO);
	...
	public void removeCashFlowItem(Long cashFlowItemId) throws EntityNotFoundException, ConstraintsViolationException {
		CashFlowItem cashFlowItem = findCashFlowItemChecked(cashFlowItemId);
		updateCashFlow(cashFlowItem, CashFlowOperation.REMOVE);
		cashFlowItemRepository.delete(cashFlowItem);
	}
	...
	public List<CashFlowItem> findByDateRange(Date start, Date end) throws InvalidDateRange {
		return cashFlowItemRepository.findByDateRange(start, end);
	}

	/**
	 * List all CashFlowItems available
	 * @return List<CashFlowItem>
	 * 
	 */
	@Override
	public List<CashFlowItem> findAll() {
		...
		cashFlowItemRepository.findAll().forEach(cfi -> list.add(cfi));
		return list;
	}
	...
	private CashFlowItem findCashFlowItemChecked(Long cashFlowItemId) throws EntityNotFoundException {
		return cashFlowItemRepository.findById(cashFlowItemId)
				.orElseThrow(() -> new EntityNotFoundException("Could not find entity with id: " + cashFlowItemId));
	}
