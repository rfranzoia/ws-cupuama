...
public interface ProductFruitPriceRepository extends CrudRepository<ProductFruitPrice, Long> {
	...
	@Query(nativeQuery = true, value = "select pfp.* " +
            ...
            "inner join fruits f on f.id = pfp.fruit_id and f.deleted = false " +
            "inner join products p on p.id = pfp.product_id and p.deleted = false " +
	...
	@Query(nativeQuery = true, value = "select pfp.* " +
			"from product_fruit_price pfp " +
			"inner join fruits f on f.id = pfp.fruit_id and f.deled = false " +
            "inner join products p on p.id = pfp.product_id and p.deleted = false " +
