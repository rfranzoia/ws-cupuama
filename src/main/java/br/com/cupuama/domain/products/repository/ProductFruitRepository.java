...
public interface ProductFruitRepository extends CrudRepository<ProductFruit, ProductFruitId> {
	...
	@Query(nativeQuery = true, value = "select pf.* " +
            ...
            "inner join fruits f on f.id = pf.fruit_id and f.deleted = false " +
            "inner join products p on p.id = pf.product_id and p.deleted = false " +
	...
	@Query(nativeQuery = true, value = "select pf.* " +
			"from product_fruit pf " +
			"inner join fruits f on f.id = pf.fruit_id and f.deled = false " +
            "inner join products p on p.id = pf.product_id and p.deleted = false " +
